...
#source("./R_config.R")
packages_to_load <- c("pcutils","ReporterScore", "iCRISPR","dplyr","reshape2","ggsci","ggpubr","RColorBrewer","cowplot","patchwork","readr","tibble","vegan","ggrepel")
# 检查并安装缺失的包
...
# 加载包
pcutils::lib_ps(packages_to_load)
#color
...
# protospacers in different levels
## arc
```{r}
"cd /Users/liuzhen/Documents/GitHub/lz_crispr/data/arc23"
"scp liuzhen@10.73.29.10:/data/home/jianglab/share/seqid2taxid_23/output/total_bigarc_out_to_arc_bac ."

library(readr)
library(dplyr)
arc_spacer_level=readr::read_delim("../data/arc23/total_bigarc_out_to_arc_bac",delim = "\t",col_names = c(
    "spacer","Genome","CRISPR","Lineage","NCBI_taxonomy","taxid",
    "sseqid","pident","length","mismath","gapopen","qstart",
    "qend","sstart","send","evalue","bitscore","database",
    "sseqid_corrected","sub_taxid","level"
  ))

# same spacer id with multiple levels: select the minimum level
arc_spacer_level <- arc_spacer_level %>%
  group_by(spacer) %>%
  filter(level == min(level)) %>%
  ungroup()

# 提取spacer和level列，并且去重
arc_level <- arc_spacer_level %>%
  select(spacer, level) %>%
  distinct()

# 统计level列的频数
level_counts <- arc_level %>%
  count(level)
...
library(ggrepel)
# 选择颜色方案，比如"Set3"，可以根据需要选择其他方案
colors <- brewer.pal(n = length(unique(level_counts$level)), name = "Set3")

# 计算百分比
level_counts <- transform(level_counts, percent = (n / sum(n)) * 100)

arc_level_p <- ggplot(level_counts, aes(x = as.factor(level), y = n, fill = as.factor(level), label = sprintf("%.1f%%", percent))) +
  geom_bar(stat = "identity",) +
  geom_text(aes(label = sprintf("%.1f%%", percent)), vjust = -0.5, size=6) +  # 调整标签位置
  theme_minimal() +
  scale_fill_manual(values = colors) +
  labs(x = "Percentage of Protospacer Levels", y = "Count", fill ="") +  #delete color legend 
  theme(panel.grid = element_blank())+
  theme_pubr(base_size=23,legend = c("right"))

arc_level_p

ggsave(filename = "2.Blast_res/arc_level_count.pdf",
       plot=arc_level_p,
       device = "pdf",
       width = 8,
       height = 7,
...
top=ceiling(nrow(gene_count)*0.1)
...
p1 <- plot_enrich_res(GO_res$RefSeq,padj_threshold = 0.05,mode = 2,facet_level = T,facet_anno = GOinfo[,c(3,1)])
...
ggsave(filename = "2.Blast_res/5.gene_inter_anno_go_RefSeq_10.pdf")
p2<-plot_enrich_res(GO_res$GeneID,padj_threshold = 0.05,mode = 2,facet_level = T,facet_anno = GOinfo[,c(3,1)])
p2

ggsave(filename = "2.Blast_res/5.gene_inter_anno_go_GeneID_10.pdf")
